...
describe('CreateButton', () => {
    ...
    test('render custom icon khi được truyền vào', () => {
        render(<CreateButton onClick={vi.fn()} icon={<span>CustomIcon</span>} />)
...
describe('NoteCard', () => {
    ...
    test('áp dụng background color từ note.color', () => {
        const { container } = render(<NoteCard note={makeNote({ color: '#ff0000' })} />)
    ...
    test('không có inline backgroundColor khi color là null', () => {
        const { container } = render(<NoteCard note={makeNote({ color: null })} />)
...
describe('NoteGrid', () => {
    test('hiển thị đúng số skeleton theo skeletonCount khi loading', () => {
        const { container } = render(<NoteGrid notes={[]} isLoading skeletonCount={3} />)
    ...
    test('render đúng số lượng NoteCard theo notes', () => {
        const notes = [makeNote({ id: 1, title: 'Note A' }), makeNote({ id: 2, title: 'Note B' })]
    ...
    test('không hiển thị empty message khi đang loading', () => {
        ...
        expect(screen.queryByText('Chưa có ghi chú nào')).not.toBeInTheDocument()
    })
})
...
describe('LoginCard', () => {
    ...
    test('validation: hiện lỗi khi email không hợp lệ', async () => {
        ...
        Object.defineProperty(emailInput, 'value', { configurable: true, writable: true, value: 'notanemail' })
    ...
    test('gọi onLogin khi form hợp lệ', async () => {
        ...
        await userEvent.type(screen.getByPlaceholderText('test@example.com'), 'user@example.com')
...
describe('RegisterCard', () => {
    ...
    test('validation: hiện lỗi khi display_name trống', async () => {
        ...
        expect(screen.getByText('Tên người dùng không được để trống')).toBeInTheDocument()
    })
    ...
    test('validation: hiện lỗi khi display_name ngắn hơn 3 ký tự', async () => {
        ...
        await userEvent.click(screen.getByRole('button', { name: /đăng ký/i }))
        expect(screen.getByText('Tên người dùng không được nhỏ hơn 3 ký tự')).toBeInTheDocument()
    })
    ...
    test('validation: hiện lỗi khi mật khẩu không khớp', async () => {
        ...
        await userEvent.click(screen.getByRole('button', { name: /đăng ký/i }))
        expect(screen.getByText('Mật khẩu không trùng khớp')).toBeInTheDocument()
    })
    ...
    test('gọi onRegister khi form hợp lệ', async () => {
        ...
        expect(onRegister).toHaveBeenCalledWith(
            expect.objectContaining({ display_name: 'TestUser', email: 'user@example.com', password: 'password123' })
        )
    })
