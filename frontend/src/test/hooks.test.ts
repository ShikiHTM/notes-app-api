...
import { useNotes, useNote, notesQueryKey, noteQueryKey } from '../hooks/Note.hook'
...
vi.mock('@tanstack/react-query', () => ({ useQuery: (...args: any[]) => mockUseQuery(...args) }))
...
describe("useToast test cases", () => {
    ...
    test.each<[string, ToastType]>([
        ...
        ['Đang xử lý', 'info'],
        ['Cảnh báo', 'warning'],
    ])('useToast – showToast("%s", "%s") gọi context đúng args', (message, type) => {
        const ctx = makeToastCtx()
        mockUseContext.mockReturnValue(ctx)
...
describe("useModal test cases", () => {
    test('useModal – throws when outside ModalProvider', () => {
        mockUseContext.mockReturnValue(undefined)
        expect(() => useModal()).toThrowError('useModal must be used within a ModalProvider')
    })
...
describe("useAuth test cases", () => {
    test('useAuth – throws when outside AuthProvider', () => {
        mockUseContext.mockReturnValue(undefined)
        expect(() => useAuth()).toThrowError('useAuth must be used within an AuthProvider!')
    })
